...
$(function() {
    ...
    var onSelectTrackNode = function(node) {
        if (node.id != -1 && node.pId == -1) { // 漫游路径对象节点
            ...
            if (track) {
                $("#divShowRoute,#btnStart,#video").removeAttr("disabled");
                ...
                if (track.Status == 0) { // stop
                    ...
                    $("#divRoleTrack").attr("disabled", "disabled");
                    $("#btnStart").text("飞行");
                    ...
                    $("#divRoleTrack").removeAttr("disabled");
                    $("#btnStart").text("暂停");
                    $("#selFlyObj").val(TrackMap[track.guid]);
                    earth.TrackControl.SetMainTrack(track.guid, 0);
            ...
            $("#btnStart,#btnStop,#divRoleTrack,#divChcekControl,#video,#divRealTimeControl").attr("disabled", "disabled");
        }
    ...
    $("#btnNew").click(function() {
        ...
        var tracks = trackManager.getTracks();
        var trackName = showModalDialog("getTrackName.html", tracks, "dialogWidth=180px;dialogHeight=90px;status=no");
        if (trackName) {
            $.parser.parse('#divTrack');
            bNew = true;
    ...
    var startFly = function(node, callback) {
        ...
        if (track) {
            if (track.Status == 0) { // stop
                earth.Event.OnDocumentChanged = function(type, newGuid) {
                    ...
                    if (type == 2) { // 飞行对象加载成功
                        ...
                        if(treeTrack.getSelectedNodes()[0].id == -1){
                            ...
                            $("#btnStop,#divRoleTrack,#divChcekControl").removeAttr("disabled");
                        }
                        $("#btnStart").text("暂停");  
                ...
                $("#selFlyObj").attr("disabled", "disabled");
                $("#btnNew,#divRoleTrack").attr("disabled", "disabled");
    ...
    $("#btnStart").click(function() {
        ...
        //free update 2014年7月24日17:00:01
