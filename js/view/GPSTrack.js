...
$(function(){
    ...
    $("#divDeleteTrack").click(function(){
        ...
        if (node && node.id) {
            if (confirm("是否确定要删除？")) {
                ...
                if(rootNode.chilren == null || rootNode.chilren.length == 0){
    ...
    $("#btnAdd").click(function(){
        ...
        var attrArr = [
            {name:"ID",value:routeGuid},
            {name:"NAME",value:objName},
            {name:"DYNAMICID",value:dynamicId},
            {name:"TYPE",value:type},
            {name:"HEIGHTTYPE",value:heightType},
            {name:"INFORMATION",value:detailInfo}
        ]
...
function setMainTrack(event, treeId, node){
    if(node){
        if(node.isParent){
            ...
            var nodeIndex = node.index;
            if(record[nodeIndex].track && record[nodeIndex].trackPoints[record[nodeIndex].trackPointIndex]){
                earth.GlobeObserver.FlytoLookat(record[nodeIndex].trackPoints[record[nodeIndex].trackPointIndex].x, record[nodeIndex].trackPoints[record[nodeIndex].trackPointIndex].y, record[nodeIndex].trackPoints[record[nodeIndex].trackPointIndex].z, 0, 90, 0, 100, 2);
            }
            //注意：此接口使用有问题，暂时注释掉
...
//显示或隐藏运动物体
function showDynamicObj(isShow){
    if(!tracks){
        return;
    }
    for(var i in tracks){
        var t = tracks[i];
        if(t){
            t.ShowBindObject(isShow);
            earth.DynamicSystem.GetSphericalObject(t.BindObject).Visibility = isShow;
        }
    }
}
...
function GPSTrack() {
    ...
    getDynamicList(function(){//获取运动对象列表
        ...
        var finishTimes = 0;//路径完成的条数,针对不开启循环
            ...
            for(var i = 0; i < record.length; i++){
                if(record[i].track && record[i].trackPoints){
                    if(isRound){//如果开启了循环则用定时器实现来回
                        var nTrackIndex = 0;
                        if(ci%((record[i].trackPoints.length - 1)*2) <= record[i].trackPoints.length - 1){
                            nTrackIndex = ci%((record[i].trackPoints.length - 1)*2);
                        }else{
                            nTrackIndex = ((record[i].trackPoints.length - 1)*2) - ci%((record[i].trackPoints.length - 1)*2);
                        }
                        ...
                        if(ci < record[i].trackPoints.length ){
                            record[i].trackPointIndex = ci;
                            record[i].track.AddGPS(record[i].trackPoints[ci].x, record[i].trackPoints[ci].y, record[i].trackPoints[ci].range, lunXunTime * 1000);
                        }else{
                            finishTimes++;
                            if(record[i].track.BindObject){
                                earth.DynamicSystem.UnLoadDynamicObject(record[i].track.BindObject);
                            }
                            earth.GPSTrackControl.DeleteTrack(record[i].track.guid);
                            record[i].track = null;
                            if(finishTimes == record.length){
...
function gpsPlay(trackObj,newGuid){
    ...
    track.AddGPS(pts[ci].x, pts[ci].y,pts[ci].range,time);
    ci++;
    track.AddGPS(pts[ci].x, pts[ci].y,pts[ci].range,time);
    ci++;
    track.AddGPS(pts[ci].x, pts[ci].y,pts[ci].range,time);
