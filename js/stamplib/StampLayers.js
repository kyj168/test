    ...
    initProjectList:function(){
            ...
            for(var i = 0; i < projectCount; i++){
                ...
                if(layerType === "Project"){  //17
                    ...
                    var  pipeTag = false;
                    pipeTag = LayerManagement.hasPipelinelayer(childLayer);//modified by zhangd 2015-03-27 
                    if(pipeTag){
...
/*
在页面初始化的时候将字段的映射地图保存为键值对
 */
var fieldMapObj = {};
function createFieldMapObj() {
    fieldMapObj['lineFieldInfo'] = {};
    fieldMapObj['pointFieldInfo'] = {};
    if (SYSTEMPARAMS.pipeFieldMap) {
        var lineFieldInfo = {};
        var pointFieldInfo = {};
        var configXML = SYSTEMPARAMS.pipeFieldMap;
        var lineData = configXML.getElementsByTagName("LineFieldInfo")[0] ? configXML.getElementsByTagName("LineFieldInfo")[0].getElementsByTagName("SystemFieldList")[0] : null;
        var pointData = configXML.getElementsByTagName("PointFieldInfo")[0] ? configXML.getElementsByTagName("PointFieldInfo")[0].getElementsByTagName("SystemFieldList")[0] : null;
        if (lineData) {
            for (var i = 0; i < lineData.childNodes.length; i++) {
                var node = lineData.childNodes[i];
                ...
                lineFieldInfo[node.getAttribute("FieldName").toUpperCase()] = {
                    CaptionName: node.getAttribute("CaptionName"),
                    StandardName: node.getAttribute("StandardName")
                };
            }
        }
        if (pointData) {
            for (var i = 0; i < pointData.childNodes.length; i++) {
                var node = pointData.childNodes[i];
                if (!("getAttribute" in node)) {
                    continue;
                }
                if (node.getAttribute("FieldName") == "") {
                ...
                pointFieldInfo[node.getAttribute("FieldName").toUpperCase()] = {
                    CaptionName: node.getAttribute("CaptionName"),
                    StandardName: node.getAttribute("StandardName")
                };
            }
        ...
        fieldMapObj = {
            lineFieldInfo: lineFieldInfo,
            pointFieldInfo: pointFieldInfo
        };
    }
};
