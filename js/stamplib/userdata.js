...
var editDataArr = self.editDataArr || parent.editDataArr;
var objArr = self.objArrs || parent.objArrs;
var earth = null;
var treePIDObject = {};
var nodeLevel = {};
var cameraArr = []; //摄像头数组
...
//二维对象：dataType == 209 || dataType == 211 || dataType == 220 || dataType == 227 
//          || dataType == 228 || dataType == 229 || dataType == 243 || dataType == 245
//几何对象：dataType == 216 || dataType == 202 || dataType == 207 || dataType == 203 ||
...
STAMP.Userdata = function (earthObj) {
    ...
    var _createParticle = function (flag) {
        ...
        earth.Event.OnCreateGeometry = function (pt, type) {
            if (pt) {
                window.showModalDialog("html/userdata/getParticleName.html", obj, "dialogWidth=160px;dialogHeight=105px;status=no");
                var name = obj.name;
                if (obj.name == "") {
                    return;
                }
    ...
    var _createPrimitives = function (flag, userdataTree) {
        ...
        if (flag == "createline") {
            ...
            function onPickObject(pObj) {
                ...
                if (pCounts == undefined) {
                    ...
                    var rValue = showModalDialog("html/userdata/lineBuffer.html", pObj, "dialogWidth=250px;dialogHeight=187px;status=no");
                    if (!rValue) {
                    ...
                    if (rValue.bufferStyle == "pingtou") {
                        bufGeoPoints = earth.GeometryAlgorithm.CreatePolygonFromPolylineAndWidth(selectedObj, rValue.leftRadius, rValue.rightRadius);
                    } else if (rValue.bufferStyle == "yuantou") {
                        bufGeoPoints = earth.GeometryAlgorithm.CreatePolygonFromPolylineAndWidthWithHead(selectedObj, rValue.leftRadius, rValue.rightRadius);
                    }
                    var pt = null;
        ...
        //创建风场立方体 
            ...
            elementBox.FillColor = parseInt("0xccc0c0c0"); //12632256;
            elementBox.EndUpdate();
            userdataArr.push(elementBox);
            //使立方体高亮
            ...
            earth.Event.OnCreateGeometry = function (pval, type) {
                if (pval) {
                    ...
                    var rValue = showModalDialog("html/userdata/3DEdit_ex.html", userdataObj, "dialogWidth=315px;dialogHeight=680px;status=no");
                    if (userdataObj.click == undefined || userdataObj.click == "false") {
                        earth.ShapeCreator.Clear();
                        return;
                    }
                    ...
                    elementVolume.LineColor = parseInt("0x" + userdataObj.linecolor.toString().substring(1).toLowerCase());
                    elementVolume.DrawFrameEnable = userdataObj.drawFrameEnable;
        ...
        } else if (flag == "createflowRoute") { //路线
            userdataObj.name = "flowRoute";
            userdataObj.type = 230;
            earth.Event.OnCreateGeometry = function (pval, type) {
                ...
                //elementPyramid.LineStyle.LineColor = parseInt('0xccff0000');
                elementPyramid.LineStyle.LineWidth = 1;
                elementPyramid.Visibility = true;
                elementPyramid.EndUpdate();
                var param = showModalDialog("html/userdata/flowroute.html", {earth: earth}, "dialogWidth=400px;dialogHeight=450px;status=no");
                earth.ShapeCreator.Clear();
                if (!param) return;
                //var model = createRoute(line, param);
                var texture = param.texture,
                    radius = param.radius,
                    path = earth.RootPath + 'userdata\\' + elementPyramid.Guid + '.usb',
                    v3 = elementPyramid.Generate_Flow_Mesh(true, texture, path, radius);
                earth.AttachObject(elementPyramid);
                ...
                userdataObj.altitude = v3.Z + radius;
                userdataObj.pathUsb = earth.RootPath + 'userdata\\' + elementPyramid.Guid + '.usb';

                var model = earth.Factory.CreateEditModelByLocal(userdataObj.guid, '', path, 1);
                model.SphericalTransform.SetLocationEx(v3.X, v3.Y, v3.Z + radius);

                model.name = "flowRoute";
            ...
            earth.Event.OnCreateGeometry = function (pval, type) {
                if (pval.Count) {
                    ...
                    userdataObj.path = earth.Environment.RootPath;
                    var rValue = showModalDialog("html/userdata/getParticleName.html", userdataObj, "dialogWidth=160px;dialogHeight=105px;status=no");
                    if (userdataObj.click == undefined || userdataObj.click == "false") {
                        earth.ShapeCreator.Clear();
                        return;
                    }
    ...
    var _createSignalSphere = function (flag) {
        ...
        earth.Event.OnCreateGeometry = function (pt) {
            ...
            if (!param) return;
            //var ss = createSignalSphere(pt, param);
    ...
    var createElementXml = function (obj, elment) {
        ...
        if (obj.type === 216 || obj.type === 202 || obj.type === 207 || obj.type === 203 ||
            ...
            if (obj.type === 207) {
                xmlData += " <LineColor>" + obj.linecolor + "</LineColor>";
                xmlData += "<DrawFrameEnable>" + obj.drawFrameEnable + "</DrawFrameEnable>";
            }
    ...
    var _editpoint = function () {
        saveTag = "save";
        var _obj = self.$.fn || parent.$.fn;
        zTree = _obj.zTree.getZTreeObj("userdataTree");
        zTree.setting.callback.onRightClick = function () {
        };
    ...
    var _deletepoint = function () {
        ...
        earth.Event.OnGeometryDeletePoint = function (v1) {
            ...
            zTree.setting.callback.onRightClick = rightClickuserdataTreeNode;
        };
        zTree.setting.callback.onRightClick = function () {
        };
        ...
        if (selObjNum <= 0 || selObjNum == "") {
            ...
            if(r){
                ...
                for(var j=0; j<selectObjs.length; j++){
                    ...
                    var node = zTree.getNodesByParam("id", thisObj.Guid)[0];
                    if(thisZTree){
    ...
    $(document).ready(function () {
        ...
        earth.Event.OnControlPointSelectChanged = function (x) {
            selectObj();
            //alert("OnControlPointSelectChanged");
        }
        ...
        for (var i = 0; i < editDataArr.length; i++) {
            ...
            for (var m = 0; m < objs.length; m++) {
                ...
                if (id == element.Guid) {
                    ...
                    // if (objitem.getAttribute("type") == "250" || objitem.getAttribute("type") == "250") {
                    //     var a = 1;
                    //     var coordinates = objitem.getElementsByTagName("Coordinates");
                    //     if (coordinates.length > 0) {
                    //         var nodeitem = coordinates[0];
            ...
            if (eleid === id) {
                ...
                if (obj.type === 216 || obj.type === 202 || obj.type === 207 || obj.type === 203 ||
                    ...
                    if (obj.type === 206) {
                        ...
                        window.showModalDialog("html/userdata/3DEdit_ex.html", obj, "dialogWidth=315px;dialogHeight=680px;status=no");
                    } else if (obj.type === 203) {//203圆柱
                        window.showModalDialog("html/userdata/3DEdit_ex.html", obj, "dialogWidth=305px;dialogHeight=610px;status=no");
                    } else {//202立方体
                ...
                if (obj.type === 280 || obj.type === 223 || obj.type === 217 || obj.type === 209 || obj.type === 244) {
                    ...
                    obj.name = currentElement.name;
                    if (obj.type === 223) {
                        var tag = docElement.getElementsByTagName("Tag")[0].text;
                        switch (tag) {
                            case "1":
                                obj.flag = "model";
                                break;
                        ...
                        // earth.UserDocument.DeleteFile(lastLink);
                        // earth.DetachObject(currentElement);
    ...
    var _editElementToEarth = function (id, obj, currentElement) {
        ...
        if (obj.type === 216 || obj.type === 202 || obj.type === 207 || obj.type === 203 ||
            ...
            if (obj.type === 207) {
                editObj.DrawFrameEnable = obj.drawFrameEnable;
                editObj.LineColor = parseInt("0x" + obj.linecolor.toString().substring(1).toLowerCase());
            }
        ...
        if (obj.type === 280 || obj.type === 223 || obj.type === 217 || obj.type === 209 || obj.type === 244) {
            if (obj.type === 223) {
                ...
                // for(var i=0; i<roofMats.Count; i++){
                //     roofMats.Items(i).DiffuseTexture =obj.roofTextureNode;
                // }
                editObj.EndUpdate();
            } else if (obj.type === 244) {
                editObj.BeginUpdate();
    ...
    var getAllIconObjs = function (currUserdataDoc) {
        ...
        for (var i = 0; i < element.length; i++) {
            ...
            if (objType === 250 || objType === 251 || objType === 252 || objType === 253 || objType === 254 || objType === 255 || objType === 256 || objType === 257 || objType === 258 || objType === 259 || objType === 260) {
                ...
                if (objType === 207) {
                    obj.LineColor = elementObj.getElementsByTagName("LineColor")[0].text;
                    obj.DrawFrameEnable = elementObj.getElementsByTagName("DrawFrameEnable").text;
                }
                ...
                var texturePath = elementObj.getElementsByTagName("TexturePath");
                var texturePathCount = texturePath.length;
                if (elementObj.getElementsByTagName("LineColor").length > 0) {
                    obj.lineColor = elementObj.getElementsByTagName("LineColor")[0].text;
                    obj.lineColor = parseInt("0x" + obj.lineColor.toString().substring(1).toLowerCase());
                }
                if (elementObj.getElementsByTagName("DrawFrameEnable").length > 0) {
                    obj.drawFrameEnable = elementObj.getElementsByTagName("DrawFrameEnable")[0].text;
                }
                var texturesArr = [];
                for (var j = 0; j < texturePathCount; j++) {
                    texturesArr.push(texturePath[j].text);
                ...
                volume.BeginUpdate();
                volume.DrawFrameEnable = obj.drawFrameEnable;
                volume.LineColor = obj.lineColor;
    ...
    var rightClickuserdataTreeNode = function (event, treeId, node) {
        ...
        if (node) {
            ...
            else if (node && node.isParent && node.name != treeRootName) {
    ...
    $("#divEditUserdata").die().live("click", function () {
        ...
        if (node && node.id) {
            ...
            if (userId) {
                ...
                //修改树节点名称
                var element = _getElementByID(userId);
                if (element) {
    ...
    function addFolder() {
        var trackName = showModalDialog("html/userdata/folderName.html", null, "dialogWidth=300px;dialogHeight=110px;status=no");
        if (trackName) {
            var guid = earth.Factory.CreateGUID();
            var zTree = $.fn.zTree.getZTreeObj("userdataTree");
            ...
            //从右键的节点作为父对象进行添加
            newNodes = zTree.addNodes(treeNode, {
                name: trackName,
                isParent: true,
                pId: treeNode.pId,
                id: guid
    ...
    function updateFolder() {
        ...
        if (treeNode && treeNode.name) {
            var trackName = showModalDialog("html/userdata/folderName.html", null, "dialogWidth=300px;dialogHeight=150px;status=no");
            if (trackName) {
                //更新节点名称
    ...
    function _updateTreeFolder(nodeId) {
        var treeNode = TreeObj.getNodesByParam("id", nodeId)[0];
        if (treeNode) {
            var trackName = showModalDialog("html/userdata/folderName.html", null, "dialogWidth=300px;dialogHeight=150px;status=no");
            if (trackName) {
                //更新节点名称
                treeNode.name = trackName;
                TreeObj.updateNode(treeNode);
                updateElementsFolder(treeNode, trackName);
                return trackName;
            }
        }
    }
    ...
    function _deleteTreeFolder(nodeId) {
        var node = TreeObj.getNodesByParam("id", nodeId)[0];
    ...
    function _deleteUserdataNode(nodeId) {
        ...
        var zTree = $.fn.zTree.getZTreeObj("userdataTree");
        var nodes = zTree.getNodesByParam("id", nodeId);
        var node = nodes[0];
        if (node && node.id) {
            //删除UI上的树节点
            zTree.removeNode(node);
    ...
    function _zTreeGetNode(nodeId, bChecked) {
        var treeNode = TreeObj.getNodesByParam("id", nodeId)[0];
        if (treeNode) {
            TreeObj.checkNode(treeNode, bChecked);
            zTreeOnCheck(null, null, treeNode);
        }
    }
    ...
    //创建信号辐射球
    userdata.createSignalSphere = _createSignalSphere;
    ...
    userdata.updateTreeFolder = _updateTreeFolder;
