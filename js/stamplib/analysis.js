...
var AnalineObj = null;
var SurfaceAreaResult = null;
var slopePolygonObj = null;  //坡度图
var lineparam=null;
var undergroundWaterObj = null;//地下水
...
STAMP.Analysis = function (earth) {
    ...
    var clear = function () {
        ...
        if (_tempLayer) {
            ...
            //earth.Factory.DestoryObject(_tempLayer);
            _tempLayer = null;
        }
        if (_tempModel) {
            earth.DetachObject(_tempModel);
            //earth.Factory.DestoryObject(_tempModel);
            _tempModel = null;
        }
        //清除坡度图
        if (slopePolygonObj) {
        ...
        if(submergePolygon){
            ...
            submergePolygon.Visibility = false;
    ...
    var _showMeasureResult = function (result) {
        var bc = '#f4faff';
        var c = SYSTEMPARAMS.balloonAlpha == 0 ? '#0075C8' : '#ff00ff';
    ...
    */
    ...
    var lineOfSight = function(startHeight, endHeight, btn){
        ...
        earth.Event.OnAnalysisFinished = function(res) {
            if (earth.htmlBallon.Guid) {
                ...
                res.ClearRes();
                resArr = [];
                if(lightSightObj.length != 0){
                    for(var i = 0; i < lightSightObj.length; i++){
                        earth.DetachObject(lightSightObj[i]);
                    }
                    lightSightObj = [];    
                }
            }
     ...
     * 天际线分析
     */
    var skyline = function (type, height, dis,deep,isunder) {
        clear();
        earth.Event.OnCreateGeometry = function (line) {
            ...
            if(isunder){
                ...
                earth.GlobeObserver.FixedPointObserveEx3(line,height,dis,0,type,true);
            }
            //earth.GlobeObserver.FixedPointObserveEx2(line.GetPointAt(0), line.GetPointAt(1), height, dis,deep, type);
            $("#btnStart").attr("disabled", "disabled");
    ...
    var buidTobuid = function () {
        ...
        earth.Event.OnSelectChanged = function (x) {
            ...
            for (var i = 0; i < selectSet.GetCount(); i++) {
                var element = selectSet.GetObject(i);
                if (element.Rtti === 223) {   //280/220    EditModel= 229,
            ...
            if (modelArr.length == 2) {
                ...
                //modelArr2[0].StopHighLight();
                //modelArr2[1].StopHighLight();
            }
    ...
    var showMoveHtml = function (anaLysisChk) {
        ...
        if (anaLysisChk === "mLineSight") {//通视分析
          ...
          height = 141;
            ...
            height = 213;
            width = 323;
        } else if (anaLysisChk === "exportUSX") {   //导出usx
        ...
        //设置剖面分析展示页面
        else if (anaLysisChk === "clipScene") {
            url = loaclUrl + "/html/analysis/clipscene.html";
            width = 300;
            height = 200;
        }
        //设置风场展示页面
        else if (anaLysisChk === "windScene") {
            url = loaclUrl + "/html/analysis/windscene.html";
            width = 300;
            height = 300;
        }
        //设置淹没分析展示页面
        else if (anaLysisChk === "waterground") {
        ...
        else if(anaLysisChk == "undergroundWater"){
            url = loaclUrl +"/html/analysis/underground_water.html";
            width = 340;
            height = 250;
        ...
        htmlBalloonMove.SetScreenLocation(locationWidth, 0);//earth.offsetHeight
    ...
    var getCircleVec3s = function(p,waterHeight){
        ...
        tran.SetLocationEx(p.Longitude,p.Latitude,p.Altitude);
        bufPolygon.BeginUpdate();

        bufPolygon.Radius = p.Radius;
    ...
    var createSubmergePolygon = function(vec3s,subwater1,waterHeight,time,isShowSide){
        ...
        if(subwater1){
            ...
            submergePolygon.SetParam(waterHeight,waterHeight,1);
        }
    ...
    function createHeatMap(filePath,radius, density, altitude, terrain,tableId){
        ...
        var fileContent = earth.UserDocument.LoadXmlFile(filePath);
        try{
            if(fileContent){
                fileContent = fileContent.split("\r\n");
                if(fileContent[0]){
                    var firstData = fileContent[0].split(" ");
                    if(firstData){
                        var longitude = firstData[0];
                        var latitude = firstData[1];
                        earth.GlobeObserver.GotoLookat(longitude, latitude, altitude, 0, 90, 0, 1000);
                    }
                }
            }
        }catch(e){
    ...
    //坡度图
    function slope(tableId) {
        analysis.clear();
        earth.Event.OnCreateGeometry = function (pVal) {
            earth.Event.OnCreateGeometry = $.noop;
            if (pVal.Count <= 2) return;
            ...
            slopePolygonObj.AltitudeType = 1;
            var tableArr = getTableArr(tableId);
            for(var i=0;i<tableArr.length;i+=2){
                var color = tableArr[i+1];
                var newColor = color.replace("#","0x");
                slopePolygonObj.AddGrade(tableArr[i],parseInt(newColor));
            }
            slopePolygonObj.EndUpdate();
            
            earth.AttachObject(slopePolygonObj);
        };
        earth.ShapeCreator.CreatePolygon();
    }
    //等高线实现方法
    ...
    var windScene = function(path,layers,x,y) {
        ...
        setTimeout(function () { //延迟否则与OnHtmlBalloonFinished冲突
            earth.Measure.WindScene(path,layers,x,y);//121.513508,31.235836
        }, 100);
    }
     //获取文件夹下子目录数量
    ...
    // analysis.undergroundWater = undergroundWater;//地下水
    // analysis.createUndergroundWater = createUndergroundWater;
