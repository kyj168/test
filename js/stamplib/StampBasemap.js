...
function loadEarthData(screen){
    ...
    var stampCAB = 'codebase="stamp2/stamp32.CAB#version=3,1,1,1"'; //32位cab包
    if (ieVersion == "Win64") {
        stampCAB = 'codebase="stamp2/stamp64.CAB#version=3,1,1,1"'; //64位cab包
    }
...
function setSync(bSync, isJinlparam) {
    ...
    if (bSync) {        //联动
        if(isJinlparam){ 
            ...
            // LayerManagement.earthArray[1].Event.OnObserverChanged = emptyFunction; 

            LayerManagement.earthArray[1].GlobeObserver.GotoLookat(LayerManagement.earthArray[0].GlobeObserver.pose.longitude, LayerManagement.earthArray[0].GlobeObserver.pose.latitude, LayerManagement.earthArray[0].GlobeObserver.pose.altitude, LayerManagement.earthArray[0].GlobeObserver.pose.heading, LayerManagement.earthArray[0].GlobeObserver.pose.tilt, LayerManagement.earthArray[0].GlobeObserver.pose.roll, 0); 
        }else{ 
...
function createEarth(data,is2d, callback) {
    ...
    seearth1.Event.OnCreateEarth = function (pval) {
        ...
        if(data){
            seearth1.Load(params.ip,data);
        }else{
            seearth1.Load(params.ip,params.screen);
        }
