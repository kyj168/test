...
function setMenuData() {
    ...
    $("#scrollDiv").height($(this).height()-$("#paramSetInner").height()-50);
...
function bRightType(type, dataType){
    ...
    if(type == 1 && (dataType == 209 || dataType == 211 || dataType == 220 || dataType == 227 
        ...
        //应急标绘
        return true;
    }else if(type == 3 && (dataType == 216 || dataType == 202 || dataType == 207 || dataType == 203 ||
        dataType == 204 || dataType == 205 || dataType == 206 || dataType == 280 || dataType == 230 ||
        dataType == 'cordon' || dataType == 210)){
        //几何对象
        return true;
    }else if(type == 4 && (dataType == 217 || dataType == 223)){
        //三维模型
        return true;
...
//删除子节点
$("#divDeleteUserdata").live("click", function(){
    var tempTreeObj = $.fn.zTree.getZTreeObj("userDataTree");
    var nodes = tempTreeObj.getSelectedNodes();
    if(nodes && nodes.length > 0){
        if (confirm("是否确定要删除?")) {
